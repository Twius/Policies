...
> **In short:** framelad has no servers and no user accounts. It collects, stores, transmits, and shares no personal data, and makes no internet connections of its own. The only network feature is optional *local link play*, device-to-device connections on your own local network that contact no servers.
...
Those five are the only permissions framelad declares for its own use. The AndroidX libraries it is built with add one more, a private signature permission that only framelad can hold and that grants no access to anything on your device.

## 5. Analytics, advertising, and tracking

framelad contains **no analytics SDKs, no advertising, and no third-party tracking**, and bundles no third-party data-collecting SDKs. The app is built on the open-source [mGBA](https://mgba.io/) emulation core, which also runs entirely on-device and collects no data. We do not profile you, and we do not sell or share personal data.
...
Data protection law gives you rights over personal data an organisation holds about you: access, correction, erasure, portability. We hold none. framelad has no server and no account system, so there is no copy on our side to hand over, correct or delete. Everything those rights would cover sits on your device, under your control: the export functions described above give you portability, and clearing the app's data or uninstalling gives you erasure.

If you bought framelad from Google Play, Google processes that purchase as a separate controller, under [Google's privacy policy](https://policies.google.com/privacy). Rights over it are exercised with Google. framelad itself never sees or handles payment details.
